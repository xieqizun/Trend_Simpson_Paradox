...
mode=$(cat "input_info.json" | python -c "import sys, json; print json.load(sys.stdin)['mode']")
...
if [ "$mode" == 1 ]
then
	python trend_simpson_linear.py 2> /dev/null 
else
	echo here
	python trend_simpson_logistic.py 2> /dev/null 
fi
