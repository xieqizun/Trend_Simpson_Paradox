...
def find_trend_simpsons_pairs(pairs):
    ...
    for var, cond in pairs:
        ...
        if(different_vals_of(cond)) is None:
            pairs.remove((var,cond)) #!# Skip the pair which the conditioning variable does not have any value
            continue
...
def chi_sq_deviance():
    #---------------------------------------------------------------------------------------------------------
    #TODO: CHECK THE chi-sq FUNCTION IS STILL CORRECT FOR LINEAR MODEL
    #---------------------------------------------------------------------------------------------------------
    print ""
    print "################################################################################"
...
def deviance_all_pairs(pairs, aggregated_vars_params, disaggregated_vars_params):
    ...
    #---------------------------------------------------------------------------------------------------------
    #TODO: CHECK THE FORMULA IS CORRECT FOR THE NULL MODEL AND THE FULL MODEL FOR BOTH AGGREGATED AND DISAGGREATED DATA
    #---------------------------------------------------------------------------------------------------------
    for var, cond in pairs:
        if(different_vals_of(cond)) is None:
            continue
        ...
        if var not in null_agg:
            ...
            #USE OF NORMAL DISTRIBUTION FORMULA TO CALCULATE THE LOGLIKELIHOOD
...
#         full_disagg_tmp = 0
        ...
        for ind in range(len(conditioning_groups) - 1):
            ...
            #SKIP THE ELEMENT THAT IS NOT BE ABLE TO BE CALCULATED
            if(sigma == 0):
                disagg_ind += 1
                continue
            #!# 
